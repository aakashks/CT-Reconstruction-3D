...
class CreateInterceptMatrix:
    ...
    def intercept_coords(self, detector_coords, dtype=torch.float32, eps=1e-10):
        ...
        C1 = (intercepts_z_plane(X, Y, Z) - intercepts_z_plane(X, Y, Z + ps)).cpu()
        C2 = (intercepts_y_plane(X, Y, Z) - intercepts_y_plane(X, Y + ps, Z)).cpu()
        C3 = (intercepts_x_plane(X, Y, Z) - intercepts_x_plane(X + ps, Y, Z)).cpu()
    ...
    def intercept_coords_rot_0(self, k, save=True, file_path='', dtype=torch.float32):
        ...
        list_sparse_matrix = [torch.empty([0, self.n ** 3, 3], dtype=dtype, device=device).to_sparse(2) for _ in
                              range(3)]
        ...
        for alpha_i in tqdm(range(self.dl), leave=False):
            for betas_i in range(self.dl // k):
                ...
                for i in range(3):
                    # TODO: throwing strange error here
                    list_sparse_matrix[i] = torch.cat([list_sparse_matrix[i], list_k_rows[i].to(device)], dim=0)
                del list_k_rows
    ...
    def intercept_matrix_rots(self, rots, file_path='', dtype=torch.float32):
        ...
        """
        create_intercept_matrix_all_rots(rots, self.n, self.phi, dl=self.dl, storage_file_path=file_path,
                                         cached_rot0=self.cache_ic_rot0_list, dtype=dtype)


def create_intercept_matrix_all_rots(rots, n, rot_angle, dl=None, storage_file_path='',
                                     rot0_file_name='intercept_coords.pt', cached_rot0=None, rot0_file_path=None,
                                     dtype=torch.float32, device=device):
    """
    directly read the rot0 intercepts file and create intercept matrix
    stores few rows of the intercept matrix generated by rotating rot0
    indexing is assumed as follows
    for each row (x, y, z).flatten()
    and rows are (phis, alphas, betas).flatten()
    """
    ...
    if rot0_file_path is None:
        rot0_file_path = storage_file_path

    if cached_rot0 is None:
        cached_rot0 = torch.load(os.path.join(rot0_file_path, rot0_file_name))

    for rotation in tqdm(rots, desc=f'Running for rotations on {device}'):
        phi = torch.tensor(rotation * rot_angle)
        c, s = torch.cos(phi), torch.sin(phi)
        # for a rotation
        rotation_matrix = torch.tensor([[c, 0, s], [0, 1, 0], [-s, 0, c]], dtype=dtype, device=device).T

        I0 = [cached_rot0[i].coalesce().to(device) for i in range(3)]
        values = [C.values().mm(rotation_matrix) for C in I0]

        I = [torch.sparse_coo_tensor(I0[i].indices(), values[i], size=(dl * dl, n ** 3, 3), dtype=dtype) for i in
             range(3)]
        intercept_coords_0 = torch.abs(torch.abs(I[0] - I[1]) - I[2])

        intercept_lengths = torch.sparse_coo_tensor(
            intercept_coords_0.indices(), torch.norm(intercept_coords_0.values(), dim=1),
            size=(dl * dl, n * n * n))

        torch.save(intercept_lengths.cpu(), os.path.join(storage_file_path, f'matrix_rot_{rotation}.pt'))

        del intercept_coords_0, intercept_lengths, rotation_matrix, I0
        # clean up memory
        gc.collect()
        torch.cuda.empty_cache()
